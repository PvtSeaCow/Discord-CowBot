...
def is_nsfw_check(message):
    if message.channel.id in ["Custom Channel ID Here","2nd Custom Channel ID Here"]:
        return True
...
def mod_or_permissions(**perms):
    def predicate(ctx):
        return role_or_permissions(ctx, lambda r: r.name in ('Bot Mod', 'Bot Admin', 'Councillor', 'Chancellor', 'Lewd', 'Lood'), **perms)

    return commands.check(predicate)

def admin_or_permissions(**perms):
    def predicate(ctx):
        return role_or_permissions(ctx, lambda r: r.name in ('Chancellor'), **perms)

    return commands.check(predicate)

def is_dev_or_perms(**perms):
    def predicate(ctx):
        return role_or_permissions(ctx, lambda r: r.name in ("Speciality", "Developer","Coder"), **perms)
